...
class Lens(ABC):
    # ... [Keep your __init__, _build_surfaces, _front_local_z, _back_local_z,
    # translate, rotate, calculate_RayIntersection, calculate_normalDirection,
    # and _find_first_face_hit EXACTLY the same] ...
    ...
    def _refract_at_surface(self, ray, surface, intersectionPoint):
        ...
        # OPTIMIZATION: Only draw debug points and update line vector if a canvas exists
        ...
        # OPTIMIZATION: Pass ray.parentCanvas, NOT self.parentCanvas
        return Ray(
            Point(new_start),
            rayDirection=newDir,
            wavelength=ray.get_Wavelength(),
    ...
    def calculate_RefractedRay(self, ray):
        ...
        if first_hit is None:
            # OPTIMIZATION: Only draw fail marks if rendering is active
            if ray.parentCanvas is not None:
                fail_mark = ray.get_StartPoint() + ray.get_Direction() * 0.5
