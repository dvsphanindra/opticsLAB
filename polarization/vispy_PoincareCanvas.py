...
class SoP:
	def __init__(self, stokes_vector, parent, label="P0", color='red'):
		...
		self.polarizationVector= self.stokes_vector[1:] / self.stokes_vector[0]
		self.label=label
		self.color=color
		# •◊×
...
class Retarder:
	...
	def retard(self, incoming_SoP):
		...
		# print(self.rotationQuaternion, si, center)
		...
		q = Quaternion(axis=(0, 0, 1), radians=angle_of_circle)
		...
		# print(start_vector, SoP_vector, np.rad2deg(angle_between_vectors))
		...
		z = r * np.sin(t)
		x = np.zeros(y.size)
		line = LinePlot((x, y, z), width=15, color=self.color, parent=self.parent)
		line.transform = scene.transforms.MatrixTransform()
		line.transform.rotate(2 * self.theta, (0, 0, 1))
		line.transform.translate(center)
...
class Polariser:
	...
	def polarise(self, incoming_SoP):
		...
		result_SoP=np.array((np.cos(gamma/2)**2, np.cos(self.theta), np.sin(self.theta),0))
		return result_SoP
...
retarder1 = Retarder(130, 45, parent=view.scene, color='teal', label='R0')

sop1=SoP(np.array((1,0,0,1)), parent=view.scene, color='white', label="SoP1")
sop2=retarder1.retard(sop1)

polariser1=Polariser(0, parent=view.scene, color='yellow', label="P1")
out_SoP=polariser1.polarise(sop2)
print("SoP out= ", out_SoP)
